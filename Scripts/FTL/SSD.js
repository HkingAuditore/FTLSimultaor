...
class SSD {
  ...
  UpdateLog(content, oldPage) {
    for (let block in this.dataLog) {
      for (let i = 0; i < this.dataLog[block].pages.length; i += 2) {
        if (SSD.BinaryToPageId(this.dataLog[block].pages[i].Read()) == "0000") {
          ...
          return this.dataLog[block].pages[i];
        }
      }
    }
    console.log("!");
  ...
  FreeSingleLog(pg){
    pg.Write("".padStart(Page.PAGE_SIZE, "0"));
    let num = pg.num.replace(/\b(0+)/gi, "");
    if(num === "")num = "0";
    this.FindPage(parseInt(num)+ 1).Write("".padStart(Page.PAGE_SIZE, "0"));
    return num;
  }
  ...
  BloomHashA(num) {
    return Math.floor(
      num / Math.floor(this.blocksTable.size / this.coldHotIndicator.length)
    );
  }
